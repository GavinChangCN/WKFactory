...
public class AttendCourseServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		AttendCourseEntity a = new AttendCourseEntity( studentNum, courseNum ) ;
		a.setAttendTime( new Timestamp( System.currentTimeMillis() ) ) ;
		
		JSONObject json = new JSONObject() ;
		// result:0-订阅成功，1-订阅失败，2已经订阅
		if( bllInterface.IsAttend( a ) ){
			json.put( "result" , 2 ) ;
		} else {
			if( bllInterface.AttendCourse( a ) ) {
				json.put( "result" , 0 ) ;
			}else {
				json.put( "result" , 1 ) ;
			}			
